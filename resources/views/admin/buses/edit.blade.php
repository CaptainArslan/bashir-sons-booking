            ...
            <form action="{{ route('admin.buses.update', $bus->id) }}" method="POST" class="row g-3">
                ...
                <div class="card">
                    <div class="card-body p-4">
                        <h5 class="mb-4">Edit Bus</h5>
                        <div class="row g-3">
                            <div class="col-md-6">
                                <label for="name" class="form-label">Bus Name <span class="text-danger">*</span></label>
                                <input type="text" class="form-control @error('name') is-invalid @enderror" id="name"
                                    name="name" placeholder="Enter Bus Name" value="{{ old('name', $bus->name) }}" required>
                                @error('name')
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control @error('registration_number') is-invalid @enderror" id="registration_number"
                                    name="registration_number" placeholder="Enter Registration Number (e.g., ABC-123)" value="{{ old('registration_number', $bus->registration_number) }}" 
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control @error('model') is-invalid @enderror" id="model"
                                    name="model" placeholder="Enter Bus Model" value="{{ old('model', $bus->model) }}" required>
                                @error('model')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control @error('color') is-invalid @enderror" id="color"
                                    name="color" placeholder="Enter Bus Color" value="{{ old('color', $bus->color) }}" required>
                                @error('color')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="col-md-6">
                                ...
                                <select class="form-select @error('bus_type_id') is-invalid @enderror" id="bus_type_id" name="bus_type_id" required>
                                    ...
                                    @foreach ($busTypes as $busType)
                                        <option value="{{ $busType->id }}" @if (old('bus_type_id', $bus->bus_type_id) == $busType->id) selected @endif>{{ $busType->name }}</option>
                                    @endforeach
                                </select>
                                @error('bus_type_id')
                                    <div class="invalid-feedback">{{ $message }}</div>
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select @error('bus_layout_id') is-invalid @enderror" id="bus_layout_id" name="bus_layout_id" required>
                                    ...
                                    @foreach ($busLayouts as $busLayout)
                                        <option value="{{ $busLayout->id }}" @if (old('bus_layout_id', $bus->bus_layout_id) == $busLayout->id) selected @endif>{{ $busLayout->name }} ({{ $busLayout->total_seats }} seats)</option>
                                    @endforeach
                                </select>
                                @error('bus_layout_id')
                                    <div class="invalid-feedback">{{ $message }}</div>
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" required>
                                    ...
                                    @foreach ($statuses as $status)
                                        <option value="{{ $status }}" @if (old('status', $bus->status->value) == $status) selected @endif>{{ ucfirst($status) }}</option>
                                    @endforeach
                                </select>
                                @error('status')
                                    <div class="invalid-feedback">{{ $message }}</div>
                            ...
                            <div class="col-12">
                                ...
                                <textarea class="form-control @error('description') is-invalid @enderror" id="description"
                                    name="description" rows="3" placeholder="Enter bus description">{{ old('description', $bus->description) }}</textarea>
                                @error('description')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
