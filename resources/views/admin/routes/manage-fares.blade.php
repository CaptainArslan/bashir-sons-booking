        ...
        <div class="col-12">
            <div class="card fare-card">
                ...
                <div class="card-body">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-3">
                                            <p class="mb-1" style="font-size: 0.85rem;">
                                                ...
                                                <span
                                                    class="badge bg-{{ $route->direction === 'forward' ? 'success' : 'warning' }} stats-badge">
                                        ...
                                        <div class="col-md-3">
                                            <p class="mb-1" style="font-size: 0.85rem;">
                                                ...
                                                <span
                                                    class="badge bg-primary stats-badge">{{ $route->base_currency }}</span>
                                            </p>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <!-- Info Box -->
                    <div class="info-box">
                        <p><i class="bx bx-info-circle me-1"></i><strong>Instructions:</strong> Set fares for all possible
                            combinations of stops on this route. The system will automatically calculate final fares based
                            on discounts. Existing fares will be updated, and new combinations will be created.</p>
                    </div>
                            ...
                            <table class="table table-bordered fare-table">
                                ...
                                <tbody>
                                    @foreach ($stopCombinations as $index => $combination)
                                        @php
                                            $fareKey =
                                                $combination['from_terminal_id'] . '-' . $combination['to_terminal_id'];
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div class="route-path">
                                                    {{ $combination['from_terminal']->name }} →
                                                    {{ $combination['to_terminal']->name }}
                                                </div>
                                                <div class="distance-info">
                                                    {{ $combination['from_terminal']->city->name }} →
                                                    {{ $combination['to_terminal']->city->name }}
                                                </div>
                                            </td>
                                            {{-- <td>
                                            ...
                                            <td>
                                                <div class="input-group">
                                                    ...
                                                    <input type="number" class="form-control fare-input base-fare"
                                            ...
                                            <td>
                                                <select class="form-select discount-type"
                                                    name="fares[{{ $index }}][discount_type]">
                                                    <option value="">None</option>
                                                    @foreach ($discountTypes as $value => $label)
                                                        <option value="{{ $value }}"
                                                            {{ old('fares.' . $index . '.discount_type', $existingFare?->discount_type?->value) == $value ? 'selected' : '' }}>
                                                            {{ $label }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                            ...
                                            <td>
                                                <input type="number" class="form-control discount-input discount-value"
                                            ...
                                            <td>
                                                <div class="input-group">
                                                    <span class="input-group-text">{{ $route->base_currency }}</span>
                                                    <input type="number" class="form-control fare-input final-fare"
                                                        name="fares[{{ $index }}][final_fare]" readonly>
                                                </div>
                                            </td>
                                            <td>
                                                <select class="form-select" name="fares[{{ $index }}][status]">
                                                    <option value="active"
                                                        {{ old('fares.' . $index . '.status', $existingFare?->status?->value ?? 'active') == 'active' ? 'selected' : '' }}>
                                                        Active</option>
                                                    <option value="inactive"
                                                        {{ old('fares.' . $index . '.status', $existingFare?->status?->value ?? 'active') == 'inactive' ? 'selected' : '' }}>
                                                        Inactive</option>
                                                </select>
                                            </td>
                                        </tr>
                                        ...
                                        <input type="hidden" name="fares[{{ $index }}][from_terminal_id]"
                                            value="{{ $combination['from_terminal_id'] }}">
                                        <input type="hidden" name="fares[{{ $index }}][to_terminal_id]"
                                            value="{{ $combination['to_terminal_id'] }}">
                                        <input type="hidden" name="fares[{{ $index }}][currency]"
                                            value="{{ $route->base_currency }}">
                                    @endforeach
                                </tbody>
                            </table>
                        </div>
