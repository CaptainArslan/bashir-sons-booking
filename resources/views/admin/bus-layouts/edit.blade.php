            ...
            <form action="{{ route('admin.bus-layouts.update', $busLayout->id) }}" method="POST" class="row g-3">
                ...
                <div class="card">
                    <div class="card-body p-4">
                        <h5 class="mb-4">Edit Bus Layout</h5>
                        <div class="row g-3">
                            <div class="col-md-6">
                                <label for="name" class="form-label">Layout Name <span class="text-danger">*</span></label>
                                <input type="text" class="form-control @error('name') is-invalid @enderror" id="name"
                                    name="name" placeholder="Enter Layout Name" value="{{ old('name', $busLayout->name) }}" required>
                                @error('name')
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" required>
                                    ...
                                    @foreach ($statuses as $status)
                                        <option value="{{ $status }}" @if (old('status', $busLayout->status->value) == $status) selected @endif>{{ ucfirst($status) }}</option>
                                    @endforeach
                                </select>
                                @error('status')
                                    <div class="invalid-feedback">{{ $message }}</div>
                            ...
                            <div class="col-12">
                                ...
                                <textarea class="form-control @error('description') is-invalid @enderror" id="description"
                                    name="description" rows="3" placeholder="Enter layout description">{{ old('description', $busLayout->description) }}</textarea>
                                @error('description')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="col-md-6">
                                ...
                                <input type="number" class="form-control @error('total_rows') is-invalid @enderror" id="total_rows"
                                    name="total_rows" placeholder="Enter total rows" value="{{ old('total_rows', $busLayout->total_rows) }}" min="1" max="50" required>
                                @error('total_rows')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="col-md-6">
                                ...
                                <input type="number" class="form-control @error('total_columns') is-invalid @enderror" id="total_columns"
                                    name="total_columns" placeholder="Enter total columns" value="{{ old('total_columns', $busLayout->total_columns) }}" min="1" max="10" required>
                                @error('total_columns')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="col-12">
                                <div class="alert alert-info">
                                    ...
                                    <strong>Note:</strong> Total seats will be calculated automatically (Rows × Columns). Current total: <strong>{{ $busLayout->total_seats }} seats</strong>
                                </div>
                            </div>
                        </div>
    ...
    function updateSeatCalculation() {
        ...
        if (alertDiv) {
            if (totalSeats > 0) {
                alertDiv.innerHTML = `
                    <i class="bx bx-info-circle me-2"></i>
                    <strong>Note:</strong> Total seats will be calculated automatically: <strong>${totalSeats} seats</strong> (${rows} rows × ${columns} columns). Current total: <strong>${currentTotalSeats} seats</strong>
                `;
            } else {
                alertDiv.innerHTML = `
                    <i class="bx bx-info-circle me-2"></i>
                    <strong>Note:</strong> Total seats will be calculated automatically (Rows × Columns). Current total: <strong>${currentTotalSeats} seats</strong>
                `;
            }
        }
    }
