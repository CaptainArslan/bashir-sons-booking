...
@section('title', 'Create Route Fare')
...
@section('content')
        ...
        <div class="ps-3">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0 p-0">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">Create Route Fare</li>
                </ol>
            </nav>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-xl-10 mx-auto">
            <form action="{{ route('admin.route-fares.store') }}" method="POST" class="row g-3">
                @csrf
                <div class="card">
                    <div class="card-body p-4">
                        <h5 class="mb-4">Create Route Fare</h5>
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <select class="form-select @error('from_stop_id') is-invalid @enderror" id="from_stop_id" name="from_stop_id" required>
                                    ...
                                    @foreach ($routeStops as $stop)
                                        <option value="{{ $stop->id }}" 
                                                ...
                                                {{ old('from_stop_id') == $stop->id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select @error('to_stop_id') is-invalid @enderror" id="to_stop_id" name="to_stop_id" required>
                                    ...
                                    @foreach ($routeStops as $stop)
                                        <option value="{{ $stop->id }}" 
                                                data-route-id="{{ $stop->route_id }}"
                                                {{ old('to_stop_id') == $stop->id ? 'selected' : '' }}>
                                            {{ $stop->terminal->name }} - {{ $stop->terminal->city->name }} ({{ $stop->terminal->code }})
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="number" class="form-control @error('base_fare') is-invalid @enderror" id="base_fare"
                                    name="base_fare" placeholder="Enter base fare amount" value="{{ old('base_fare') }}" 
                            ...
                            <div class="col-md-6" id="discount_value_container" style="display: none;">
                                ...
                                <input type="number" class="form-control @error('discount_value') is-invalid @enderror" id="discount_value"
                                    name="discount_value" placeholder="Enter discount value" value="{{ old('discount_value') }}" 
                            ...
                            <div class="col-12">
                                <div class="d-flex justify-content-end gap-2">
                                    ...
                                    <button type="submit" class="btn btn-primary px-4">
                                        <i class="bx bx-save me-1"></i>Create Route Fare
                                    </button>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function filterStopsByRoute() {
                ...
                // Reset selections if they don't belong to selected route
                if (fromStopSelect.value && fromStopSelect.querySelector(`option[value="${fromStopSelect.value}"]`).dataset.routeId !== selectedRouteId) {
                    fromStopSelect.value = '';
                }
                if (toStopSelect.value && toStopSelect.querySelector(`option[value="${toStopSelect.value}"]`).dataset.routeId !== selectedRouteId) {
                    toStopSelect.value = '';
                }
            }
